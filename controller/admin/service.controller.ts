...
export const store = async (req: Request, res: Response) => {
  try {
    ...
    if (req.body.images) {
      ...
      imagesData = files.map((file: any) => file.path || file.filename);
    } else if ((req as any).file) {
      // Trường hợp dùng Multer upload 1 ảnh đơn
      imagesData = [(req as any).file.path || (req as any).file.filename];
    }
...
console.log((service as any).images);
        const imageArray = (service as any).images || [];
